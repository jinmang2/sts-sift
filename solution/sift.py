...
class PerturbationLayer(torch.nn.Module):
    ...
    def adversarial_(self, adversarial: bool = True):
    ...
    def update_delta(self, requires_grad: bool = False) -> bool:
        if not self.adversarial_mode:
            return True
        if self.delta is None:
            delta = torch.clamp(
...
def hook_sift_layer(
    ...
    learning_rate: float = 1e-4,
    init_perturbation: float = 1e-2,
    target_module: Union[str, torch.nn.Module] = 'embeddings.LayerNorm',
    ...
    if isinstance(target_module, str):
        ...
        assert isinstance(target_module, torch.nn.Module), (
...
class AdversarialLearner:
    ...
    """
    def __init__(self, model: torch.nn.Module, adv_modules: List[torch.nn.Module] = None):
    ...
    def loss(
        ...
        target: torch.Tensor,
        logits_fn: Callable,
        loss_fn: Union[str, Callable] = 'symmetric-kl',
        *wargs,
        **kwargs,
    ) -> torch.Tensor:
        """
        ...
        if isinstance(loss_fn, str):
            loss_fn = perturbation_loss_fns.get(loss_fn, None)
            assert loss_fn is not None, (
                "`loss_fn` must to be one of these [symmetric-kl, k1, mse], "
                "or a function you define yourself."
            )
...
def symmetric_kl(logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
...
def kl(logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    logit_stu = logits.view(-1, logits.size(-1)).float()
    logit_tea = target.view(-1, target.size(-1)).float()
    logprob_stu = F.log_softmax(logit_stu, -1)
...
def mse(logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    logit_stu = logits.view(-1, logits.size(-1)).float()
    logit_tea = target.view(-1, target.size(-1)).float()
